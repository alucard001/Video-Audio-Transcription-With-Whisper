...
def transcribe_audio(
    ...
    Args:
        ...
        backend: "faster-whisper" or "openai-whisper"
    Returns:
    ...
    if backend == "openai-whisper":
        ...
        # Handle MPS device for OpenAI Whisper
        import torch
        ...
        if device == "mps":
            if torch.backends.mps.is_available() and torch.backends.mps.is_built():
                whisper_device = "mps"
                if verbose:
                    print("Using MPS device for OpenAI Whisper.")
            else:
                whisper_device = "cpu"
                if verbose:
                    print("MPS device not available, falling back to CPU for OpenAI Whisper.")
        model = openai_whisper.load_model(model_size, device=whisper_device)
        if verbose:
        ...
        results = []
        for seg in segments:
            results.append((seg["text"], seg["start"], seg["end"]))
        return results
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Transcribe video files to text using faster-whisper or OpenAI Whisper"
    )
    parser.add_argument("input", help="Path to video or audio file")
    parser.add_argument("-o", "--output", help="Path to save transcript")
    parser.add_argument("-m", "--model", default="base", help="Whisper model size (default: base)")
    parser.add_argument("-l", "--language", default=None, help="Language code (auto-detect if not provided)")
    parser.add_argument("-t", "--timestamps", action="store_true", help="Include timestamps in output")
    parser.add_argument("-d", "--device", default="cpu", help="Device to use for inference (cpu, cuda, mps)")
    parser.add_argument("-c", "--compute-type", default="float16", help="Compute type for the model")
    parser.add_argument("-k", "--keep-audio", action="store_true", help="Keep extracted audio file")
    parser.add_argument("-v", "--verbose", action="store_true", help="Print progress information")
    parser.add_argument("-b", "--backend", default="faster-whisper", choices=["faster-whisper", "openai-whisper"], help="Backend to use: faster-whisper or openai-whisper")
    args = parser.parse_args()
    output_path = transcribe_video(
        args.input,
